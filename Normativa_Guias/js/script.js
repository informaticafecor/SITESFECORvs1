...
function updateResultsCounter() {
    // Tarjetas visibles (ya filtradas)
    const visibleCards = document.querySelectorAll('.news-card.show').length;

    // Total según filtros activos (especialmente año)
    const totalCards = Array.from(document.querySelectorAll('.news-card')).filter(card => {
...
function selectYear(year) {
    ...
    // Quitar activo a todos los botones de año
    document
        .querySelectorAll('.date-filter-group:first-child .date-btn')
    ...
    // Desactivar todos los botones de mes
    document
        .querySelectorAll('#monthGroup .date-btn')
        .forEach(btn => btn.classList.remove('active'));
